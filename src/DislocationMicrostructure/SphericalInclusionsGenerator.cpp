...
SphericalInclusionsGenerator::SphericalInclusionsGenerator(const SphericalInclusionDensitySpecification& spec,MicrostructureGenerator& mg)
{
...
//        const int phaseIDs(this->parser.readScalar<int>("phaseID",true));
        ...
        std::lognormal_distribution<double> distribution(log(spec.diameterLognormalDistribution_M/spec.diameterLognormalDistribution_A),spec.diameterLognormalDistribution_S);

        const VectorDimD patternVector(spec.patternVector_SI/mg.ddBase.poly.b_SI);
        ...
        while(density<spec.targetDensity)
        {
            ...
            const double diameter = distribution(generator)*spec.diameterLognormalDistribution_A/mg.ddBase.poly.b_SI;
            ...
            if(applyPattern)
            {
                const VectorDimD globalVector(mg.ddBase.poly.grain(grainID).singleCrystal->C2G*patternVector);
...
SphericalInclusionsGenerator::SphericalInclusionsGenerator(const SphericalInclusionIndividualSpecification& spec,MicrostructureGenerator& mg)
{
...
//    }
    ...
    bool SphericalInclusionsGenerator::generateSingle(MicrostructureGenerator& mg,const VectorDimD& C,const double& R, const Eigen::Matrix<double,1,dim*dim>& eTrow, const double& vrc,const int&type, const bool& allowOutside, const bool& allowOverlap)
    {
