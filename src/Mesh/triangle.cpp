...
/*  org(abc) -> a         destT(abc) -> b              apex(abc) -> c         */
/*                                                                           */
...
#define  markT(osub)  (* (int *) ((osub).ss + 8))
...

{
  ...
  printf("subsegment x%lx with orientation %d and mark %d:\n",
         (unsigned long) s->ss, s->ssorient,  markT(*s));
...

{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    if (splitseg != (struct osub *) NULL) {
      ...
      insertsubseg(m, b, &newbotright,  markT(*splitseg));
      ...
      if (vertexmark(newvertex) == 0) {
        setvertexmark(newvertex,  markT(*splitseg));
      }
    }
...

{
  ...
  destT(*firstedge, rightbasevertex);
  if (b->verbose > 2) {
  ...
  destT(besttri, bestvertex);
  otricopy(besttri, testtri);
  bestnumber = 1;
  for (i = 2; i <= edgecount - 2; i++) {
    onextself(testtri);
    destT(testtri, testvertex);
...

{
  ...
  destT(splaytree->keyedge, checkvertex);
  if (checkvertex == splaytree->keydest) {
    ...
    destT(child->keyedge, checkvertex);
...

{
  ...
  destT(*newkey, newsplaynode->keydest);
...
*/
        ...
        maketriangle(m, b, &lefttri);
        maketriangle(m, b, &righttri);
        destT(farrighttri, connectvertex);
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (checktri.tri != m->dummytri) {
        destT(triangleloop, tdest);
        apex(triangleloop, tapex);
        /* Look for other triangles that share an edge. */
        do {
          destT(checktri, checkdest);
...

{
  ...
  setvertexmark(newvertex,  markT(*splitsubseg));
      ...
      if ((neighbor.tri == m->dummytri) || infected(neighbor)) {
        ...
        if (neighborsubseg.ss == m->dummysub) {
          ...
          *deadtriangle = neighbor.tri;
          ...
          if (markT(neighborsubseg) == 0) {
            setmark(neighborsubseg, 1);
          }
          org(neighbor, norg);
          destT(neighbor, ndest);
  ...
  while ((m->badsubsegs.items > 0) && (m->steinerleft != 0)) {
    ...
    while ((encloop != (struct badsubseg *) NULL) && (m->steinerleft != 0)) {
      ...
      if (!deadsubseg(currentenc.ss) &&
        ...
        setvertexmark(newvertex,  markT(currentenc));
        setvertextype(newvertex, SEGMENTVERTEX);
        if (b->verbose > 1) {
          printf(
  "  Splitting subsegment (%.12g, %.12g) (%.12g, %.12g) at (%.12g, %.12g).\n",
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        if (b->usesegments) {
          ...
          if (checkmark.ss != m->dummysub) {
            setvertexmark(newvertex,  markT(checkmark));
            setvertextype(newvertex, SEGMENTVERTEX);
          }
        }
        if (b->verbose > 1) {
...

{
  ...
  while (subsegloop.ss != (subseg *) NULL) {
    ...
    if (!b->nobound) {
      /* Copy the boundary marker. */
      smlist[subsegnumber - b->firstnumber] =  markT(subsegloop);
    }
#else /* not TRILIBRARY */
    ...
    if (b->nobound) {
      ...
      fprintf(outfile, "%4ld    %4d  %4d    %4d\n", subsegnumber,
              vertexmark(endpoint1), vertexmark(endpoint2),  markT(subsegloop));
    }
#endif /* not TRILIBRARY */
...
#ifdef TRILIBRARY
              emlist[edgenumber - b->firstnumber] =  markT(checkmark);
#else /* not TRILIBRARY */
              fprintf(outfile, "%4ld   %d  %d  %d\n", edgenumber,
                      vertexmark(p1), vertexmark(p2),  markT(checkmark));
#endif /* not TRILIBRARY */
            }
          } else {
#ifdef TRILIBRARY
