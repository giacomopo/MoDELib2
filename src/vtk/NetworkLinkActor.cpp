...
namespace model
{
    ...
    NetworkLinkActor::NetworkLinkActor(vtkGenericOpenGLRenderWindow* const renWin,vtkRenderer* const renderer,const DefectiveCrystal<3>& defectiveCrystal_in) :
    ...
    {// https://stackoverflow.com/questions/6878263/remove-individual-points-from-vtkpoints
        ...
        if(dislocationNetwork)
        {
            ...
            for (const auto& link : dislocationNetwork->networkLinks())
            {
                
                

                vtkSmartPointer<vtkLine> line(vtkSmartPointer<vtkLine>::New());
                line->GetPointIds()->SetId(0, link.second.lock()->source->networkID()); // the second 0 is the index of the Origin in linesPolyData's points
                line->GetPointIds()->SetId(1, link.second.lock()->  sink->networkID());
                ...
                else
                {
                    ...
                    else if(link.second.lock()->isGrainBoundarySegment())
                    {
                    ...
                    else
                    {
                        ...
                        radii->InsertNextValue(link.second.lock()->burgers().norm()*sliderLinksRadius->value());
                    }
                    
                    
//                    if(link.second.meshLocation!=0)
...
//                        cells->InsertNextCell(line);
//                        if(link.second.meshLocation==2 /*&& blackGrainBoundarySegments*/)
//                        {
...
//                auto itSource(configFields.configIO.nodeMap().find(link.second.sourceID)); //source
//                if(itSource!=configFields.configIO.nodeMap().end())
//                {
//                    auto   itSink(configFields.configIO.nodeMap().find(link.second.sinkID)); //sink
...
//                        const double burgersNorm(link.second.b.norm());
//                        if(burgersNorm>FLT_EPSILON)
//                        {
//                            Eigen::Matrix<int,3,1> colorVector=computeColor(link.second.b,chord,link.second.n);
//                            unsigned char lineClr[3]={(unsigned char) colorVector(0),(unsigned char) colorVector(1),(unsigned char) colorVector(2)};
//                            
//                            if(link.second.meshLocation!=0)
//                            {// 0=MeshLocation::insideMesh
    ...
    Eigen::Matrix<int,3,1> NetworkLinkActor::computeColor(const VectorDim& burgers, const VectorDim& chord, const std::shared_ptr<SlipSystem>& slipSystem) const
    {
        ...
        switch (linksColorBox->currentIndex())
        {
            ...
            case 1:
            {
                if(slipSystem)
                {
                    clrVector=slipSystem->unitNormal;
                }
                break;
            }
...
//                const bool isSessile((slipSystem));
                clrVector(0)= slipSystem? 1.0 : 0.1;
                clrVector(1)= slipSystem? 0.5 : 0.4;
                clrVector(2)= slipSystem? 0.0 : 0.9;
                break;
            }
